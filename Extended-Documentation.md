...
- **Output Structure**:
  - **Initial Response**: For any user query, the model responds with what foundational concept needs to be understood first. For example, for a query about Chat GPT, it might suggest understanding the concept of deep learning before exploring Chat GPT in detail.
...
- **Description**: A structured document that outlines a series of educational topics in a sequential manner. It's being made manually after 1st step.
...
- **Functionality**:
  - **Content Transformation**: This GPT transforms the structured content from current topic in the "Whole Chain" document into a set of research questions. These questions are tailored to facilitate deeper exploration and understanding of the topic.
  - **Prioritization and Roadmap**: GPT adheres to a prioritization system within the "Whole Chain." It ensures that the current topic (with the highest priority) is directly addressed through the questions, indirectly leading to the next topic and ultimately contributing to the final goal of covering the last content in the last topic video.
  - **Example-Based Guidance**: For current section of the "Whole Chain" content, the model generates a series of questions that can and will be queried on YouTube. These questions aim to delve into specific aspects of the topic.
- **Output Structure**:
  - The output is a set of research questions for each point in the "Current Content" of a topic, reflecting the priority and roadmap of the "Whole Chain."
  - Each question is formulated to encourage exploration and understanding suitable for video content creation.
- **Task Execution**: GPT follows a structured approach to create questions:
...
- **Process**: Involves researching answers to the generated questions on YouTube. It is done 'by hand' to keep control over quality.
...
### 01_download_and_transcribe.py Script - shoutout for transcription step idea - [Const-me](https://github.com/Const-me)
...
- **Functions**:
  - `read_youtube_api_key`: Reads the YouTube API key from a file (needed for URL-mapping).
  - `create_youtube_service`: Creates a YouTube service object for API interactions (for URL-mapping).
  - `get_playlist_title`: Retrieves the title of a YouTube playlist (for URL-mapping).
  - `download_videos`: Downloads videos from YouTube as MP3 files.
  - `read_problematic_chars`: Reads a list of problematic characters for text cleaning (there was an issue with .zipping files).
  - `similar_file_exists`: Checks for the existence of similar files in the transcripts folder (I don't want files to be processed twice).
  - `split_audio`: Splits audio files into manageable chunks (if something goes wrong, to not loose progress).
  - `safe_file_read`: Safely reads files, handling errors.
  - `timestamp_to_ms`: Converts timestamps to milliseconds.
  - `ms_to_timestamp`: Converts milliseconds to timestamp format (similar functions, I had problem, that every each next chunk was starting from 00:00:00).
  ...
  - `create_video_url_mappings`: Creates mappings of video URLs for each playlist (this URL-mapping thing is needed for 'Sources' step).
...
- **Purpose**: Removes timestamps and unnecessary content from the transcripts, making them cleaner and more readable (User should .zip transcripts for future steps, so now timestamps are no more needed).
- **Function**:
...
- **Purpose**: Creates SPRs (Sparse Priming Representations) for future tasks.
- **Functions**:
...
- **Purpose**: Combines and mixes all SPRs into a single, consolidated file. This mixing file thing was crucial to force GPT to read whole file, not just beginning and end (LLMs are known for being lazy :D)
- **Function**:
...
- **Role**: Specialized in creating presentation outline tailored to specific educational topics outlined in the "Whole Chain" document.
- **Process**: 
  - **Input and Sources**: GPT utilizes information from multiple sources - the "Whole Chain" document, "SAINT FILE" (that is main file from Knowledge Base), and predefined topic-related questions - to construct a comprehensive presentation outline.
...
- **Functionality**:
  - **Structured Slide Creation**: The model is tasked with creating a 30-slide presentation. Each slide is dedicated to addressing different aspects of the CURRENT TOPIC, following a precise format:
    - Slide 1: Title slide featuring the CURRENT TOPIC.
    - Slide 2: An overview of CURRENT CONTENT.
    - Slides 3-27: These slides are structured to answer the CURRENT TOPIC QUESTIONS, with each segment of questions being addressed in a group of slides. The content on these slides is derived from the SAINT FILE, with each slide containing an original explanation and a direct quote from the file.
  - **Summary and Conclusion**: The presentation includes summary slides synthesizing the key concepts discussed (Slides 28, 29) and a final 'Thank You' slide, inviting further questions or discussion.
- **Rules and Guidelines**:
  ...
  - GPT must not alter the given questions and should ensure that the presentation seamlessly integrates each part, maintaining coherence and logical flow.
- **Purpose and Goals**:
...
- **Functionality**:
  ...
  - **Detailed Output**: Provides a structured output listing each quote and its best-matched transcript segment, including the specific file name and line number(s). 
- **Rules and Guidelines**:
  - GPT must ensure that each quote is matched with a segment from the transcripts that closely relates to or conveys the same meaning.
...
- **Purpose**: Extracts .zip and processes exact timestamps from transcripts and matched quotes to create a structured timeline.
- **Functions**:
...
- **Functions**:
  ...
  - `similar`: Calculates the similarity score between two titles (there is threshold, I had to do it this way, because titles vary a bit - we cleaned unwanted chars from titles earlier).
...
- **Functionality**: 
  - **Extraction of Key Concepts**: GPT analyzes the presentation slides, focusing on key nouns and concepts relevant to the topic.
...
- **Output Structure**: 
  - A list of 30 nouns, each representing a key concept from the presentation slides. These nouns are used as variables for illustration prompts (next step).
- **Guidelines**: 
...
- **Task Execution**: GPT undertakes a structured approach to develop the narration, ensuring it aligns with the presentation's content, style, and educational goals.
...
- **Process**: 
  - **Utilizing Presentation Logic**: The presentation is manually created using the structured content from the `presentation_logic` file. This involves selecting relevant information and matching them with the appropriate visuals and narration.
...
- **Editing**:
  - **DaVinci Resolve**: Post-recording, the video is edited using DaVinci Resolve, a professional video editing software. This step involves refining the video for quality and ensuring synchronization of audio, visuals, and narration.
